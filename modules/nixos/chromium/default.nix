  ...
  config = lib.mkIf config.modules.chromium.enable {
    ...
    programs.chromium =
      ...
      rec {
        ...
        defaultSearchProviderSuggestURL = "http://localhost:${builtins.toString searx.port}/search?q={searchTerms}";
        defaultSearchProviderSearchURL = "http://localhost:${builtins.toString searx.port}/search?q={searchTerms}";

        homepageLocation =
          if searx.enable then
